        ...
        try {
            ...
            if (req.body.phase.length > 0) {
                try {
                    ...
                    const [newPhase] = await Phase.writeEventPhase(phaseArr);
            ...
            if (req.body.booking.length > 0) {
                try {
                    ...
                    const [bookEquip] = await BookEquipment.addEventEquip(bookEquipArr);
                    console.log("result booked equipment:", bookEquip);
                ...
                try {
                    ...
                    const [bookCalendar] = await BookCalendar.writeToCalendar(bookCalendarArr);
                    console.log("result bookCalendar:", bookCalendar);

                } catch (error) {
                    console.log("error:", error);
                    res.status(500).json({ msg: "We have problems with write equipment to `t_booking_calendar` table" });
                    return {
                        error: true,
                        message: 'Error from database'
                    }
            ...
            return res.status(200).json([{ msg: `Мероприятие успешно создано. idEvent = ${req.body.id}` }, req.body]);
            ...
            //         }
            //     }
            //     res.status(200).json([{ msg: `Мероприятие успешно создано. idEvent = ${eventRow[0]}` }, req.body]);
